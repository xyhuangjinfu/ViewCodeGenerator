...
/**
 * 用注释(Comment)过滤后，生成的Field列表
 * @author huangjinfu
 *
 */
public class FieldParserDom4jByComment implements IFieldParser {
    
    private Queue<Node> mNodeQueue = new LinkedList<Node>();
    private List<Field> mFields = new ArrayList<Field>();
    private Comment mCurrentComment = null;
        ...
        if (node instanceof Comment) {
            ...
            if (e.attributeValue("id") != null) {
                if (mCurrentComment != null) {
                    ...
                    field.setAnnotation(mCurrentComment.getStringValue());
                    mFields.add(field);
